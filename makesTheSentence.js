...
function makesTheSentence(characterArray, sentenceString) {
    const removeWhiteSpace = sentenceString.replace(/\s/g, "");
    const sortedArray = characterArray.sort().join("")
    const sortedString = removeWhiteSpace.split("").sort().join("")

    return sortedArray === sortedString
